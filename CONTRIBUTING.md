...
Accounts with three open pull requests may continue submitting changes, but additional submissions may be placed behind contributions from other authors in the review queue. Coding agents acting on a human's behalf should disclose this possibility and obtain the filer's confirmation before opening another pull request. This throttle applies to human-directed contributions; it does **not** apply to the repository's own agentic maintenance workflows (`add-community-extension`, `add-community-preset`, `add-community-bundle`, and `bug-fix`), which file validated PRs through the automated pipeline and must always open their PR regardless of the filing identity's open-PR count.
